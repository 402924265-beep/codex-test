...
export const ADMIN_DRIVER_MATRIX = {
  ...
  canteen: [driver("headcount", "适用用餐人数", "当月在岗及适用人员", "人力资源", "HR人员系统"), driver("workdays", "工作日", "公司年度日历", "人力资源", "系统日历"), driver("mealPrice", "餐费单价", "有效食堂合同单价", "间接采购", "采购合同")],
...
export const ADMIN_BUDGET_DATA = {
  ...
  categories: [
    ...
    },
    {
      ...
      formula: "（用餐适用人数+1% Overtime）×当月工作日×餐费单价；不含周日加班", drivers: "已保存的DW员工预测、已保存的DW行政出勤、工作日、餐费单价", allocation: "仅当前月系统计算；其他月份保留Excel",
      standards: [{ key: "headcount", label: "适用人数", value: "DW已保存员工预测 + DW已保存行政出勤", unit: "人/月", provider: "人力资源" }, { key: "workdays", label: "工作日", value: "14–26", unit: "天/月", provider: "人力资源" }, { key: "mealPrice", label: "餐费单价", value: "185.81 / 214.44", unit: "TRY/人天（1–6月 / 7–12月）", provider: "间接采购" }]
    },
    {
      ...
      formula: "乘车适用人数×工作日×人均班车单价；人均单价=线路成本÷(16×72%)×2；不含周日加班", drivers: "已保存的DW员工预测、DW行政出勤（不含司机/供应商/访客）、工作日、线路成本", allocation: "仅当前月系统计算；其他月份保留Excel"
    },
    ...
    { id: "uniforms", group: "employeeService", accountCode: "6666010314", label: "工作服", sourceLabel: "UNIFORMS", annualTry: 3252569.63, monthlyTry: average(3252569.63), ready: true, formula: "BC/ WC/ SUB按原Excel物品矩阵计算年度费用；仅固定100件外套×1.2；当前月按年度费用÷12", drivers: "已保存的DW员工预测、DW行政出勤SUB类别、物品单价、发放数量、固定特殊装备", allocation: "仅当前月系统计算；其他月份保留Excel", standards: [{ key: "headcount", label: "人员结构", value: "BC=直接+间接；WC=白领；SUB=废弃物+食堂+清洁+安保；共享人员待定", unit: "人", provider: "人力资源" }, { key: "itemPrice", label: "物品单价", value: "413–3,949.53", unit: "TRY/件", provider: "间接采购" }, { key: "issueQty", label: "发放数量", value: "1 / 2 / 12", unit: "次（按物品）", provider: "行政部门" }, { key: "surcharge", label: "外套系数", value: "仅固定100件外套×1.2", unit: "源表固定系数", provider: "行政部门" }] },
